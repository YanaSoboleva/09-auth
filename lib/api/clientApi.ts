...
import { User } from '@/types/user'; // Рекомендую імпортувати User з /types/user, де є аватар
import { NoteTag } from '@/lib/api/api';
...
/**
 * ПЕРЕВІРКА СЕСІЇ
 * ВИПРАВЛЕНО: Використовується правильний ендпоінт /auth/session
 */
export const checkSession = async () => {
...
/**
 * ОТРИМАННЯ ПРОФІЛЮ
 * Ендпоінт /users/me є правильним для отримання даних поточного юзера
 */
export const getMe = async (): Promise<User> => {
  const { data } = await api.get('/users/me');
  return data;
